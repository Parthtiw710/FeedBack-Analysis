...
import { getApiKeysFn, createApiKeyFn, deleteApiKeyFn } from "../server/functions/keys";
...
function NavLink({ to, className, children, end, ...props }: NavLinkProps) {
  return (
    <Link to={to} activeOptions={{ exact: end }} {...props}>
      {({ isActive }) => {
        const resolvedClass = typeof className === 'function' ? className({ isActive }) : className;
        return <span className={resolvedClass}>{children}</span>;
      }}
    </Link>
  );
}
...
function Sidebar({ sidebarOpen, setSidebarOpen, variant = 'default' }: SidebarProps) {
  ...
  const handleCreateKey = async () => {
    ...
    try {
      ...
      setKeyError(err.message || 'Error generating API key');
      setTimeout(() => setKeyError(''), 4000);
    }
  };

  const handleRevokeKey = async (keyToDelete: string) => {
    setKeyError('');
    try {
      const data = await deleteApiKeyFn({ data: { apiKey: keyToDelete } });
      if (data.success) {
      ...
      setKeyError(err.message || 'Error revoking API key');
      setTimeout(() => setKeyError(''), 4000);
    }
  };
  ...
  return (
    <div className="min-w-fit">
      ...
      <div
        id="sidebar"
        ref={sidebar}
        className={`flex flex-col absolute z-40 left-0 top-0 lg:relative lg:z-40 lg:left-auto lg:top-auto lg:translate-x-0 h-dvh overflow-y-auto no-scrollbar w-64 shrink-0 bg-white dark:bg-gray-800 p-4 transition-all duration-200 ease-in-out ${sidebarOpen ? "translate-x-0" : "-translate-x-64"
            ...
            <ul className="space-y-1">
              ...
              <li>
                <NavLink
                  ...
                  to="/"
                  className={({ isActive }) =>
                    `flex items-center w-full px-3 py-2 rounded-lg font-medium text-sm transition duration-150 ${isActive
          ...
          <div className="pt-4 border-t border-gray-200 dark:border-gray-700/60 mt-4">
            ...
            <div className="px-3">
                  ...
                  {apiKeys.map((key, index) => (
                    <div key={index} className="flex items-center justify-between gap-1 p-2 bg-gray-50 dark:bg-gray-900/60 border border-gray-100 dark:border-gray-800 rounded-lg text-xs">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        ...
                        <button
                          onClick={() => { if (!isTestClient) handleRevokeKey(key); }}
                          disabled={isTestClient}
                          className={`transition ${isTestClient ? 'text-gray-300 dark:text-gray-700/60 cursor-not-allowed' : 'text-gray-400 hover:text-rose-500 dark:hover:text-rose-450'}`}
                          title={isTestClient ? "Revoking API keys is disabled in the simulated test environment" : "Revoke Key"}
                        >
                          <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24">
              ...
              <button
                ...
                disabled={isTestClient}
                className={`w-full py-2 px-3 text-white font-bold text-xs rounded-lg shadow-sm transition flex items-center justify-center gap-1.5 ${isTestClient
