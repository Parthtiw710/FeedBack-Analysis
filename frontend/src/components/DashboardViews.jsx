...
// Global memory cache for all API requests to ensure instant tab switching and smooth loading
export const apiCache = {};

export async function fetchWithCache(url, force = false) {
  if (!force && apiCache[url]) {
    return apiCache[url];
  }
...
function FormTypePills({ formTypes, activeFilter, onFilterChange }) {
  return (
...
export function SubmissionsView() {
  ...
  const fetchData = async (force = false) => {
    setLoading(true);
    try {
      const data = await fetchWithCache('/submissions', force);
      setRows(data.rows || []);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm(`Delete submission #${id}?`)) return;
    try {
      const res = await fetch(`/submissions/${id}`, { method: 'DELETE' });
      if (res.ok) {
        const updated = rows.filter(r => r.id !== id && r.submission_id !== id);
        setRows(updated);
        if (apiCache['/submissions']) {
          apiCache['/submissions'].rows = updated;
        }
        setToastMsg(`Successfully deleted #${id}`);
  ...
  const formTypes = [...new Set(rows.map(r => r.form_type).filter(Boolean))].sort();
  const filteredRows = activeFilter === 'all' ? rows : rows.filter(r => r.form_type === activeFilter);
  ...
  return (
    <div className="bg-white dark:bg-gray-800 shadow-sm rounded-xl border border-gray-200 dark:border-gray-700 p-5">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6">
        <div>
          <h2 className="text-lg font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2">
            ...
            <span className="text-xs bg-violet-100 dark:bg-violet-500/20 text-violet-600 dark:text-violet-400 px-2 py-0.5 rounded-full">{filteredRows.length} entries</span>
          </h2>
        ...
        <button onClick={fetchData} className="mt-2 sm:mt-0 p-2 bg-gray-50 hover:bg-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600 rounded-lg text-gray-600 dark:text-gray-300 transition">
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 1-1 20H4M20 20v-5h-.581m0 0a8.003 8.003 0 01-15.357-2" /></svg>
        </button>
      </div>

      {toastMsg && (
        <div className="mb-4 p-3 bg-red-100 dark:bg-red-500/10 text-red-700 dark:text-red-400 text-xs font-semibold rounded-lg">
          {toastMsg}
        </div>
      )}
      ...
      {loading ? <LoadingState /> : filteredRows.length === 0 ? <EmptyState message="No submissions matching the filter." /> : (
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">
                ...
                <th className="py-3 px-2">Form Type</th>
                <th className="py-3 px-2">Score</th>
                <th className="py-3 px-2">NPS Segment</th>
                <th className="py-3 px-2">Category</th>
                <th className="py-3 px-2">Sentiment</th>
                <th className="py-3 px-2">Payload Fields</th>
                <th className="py-3 px-2">Processed At</th>
                <th className="py-3 px-2">Action</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-150 dark:divide-gray-700/55 text-sm text-gray-600 dark:text-gray-300">
              {filteredRows.map((r) => {
                const sc = r.score_primary;
                const seg = r.nps_segment || r.priority || '';
                return (
                  <tr key={r.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-700/10">
                    <td className="py-3 px-2 font-mono text-xs text-gray-400">#{r.submission_id || r.id}</td>
                    <td className="py-3 px-2">
                      <span className="px-2 py-0.5 bg-gray-100 dark:bg-gray-700 rounded text-xs font-medium text-gray-700 dark:text-gray-300">{r.form_type}</span>
                    </td>
                    <td className="py-3 px-2">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${scoreClass(sc)}`}>{sc !== null ? sc : '—'}</span>
                    </td>
                    <td className="py-3 px-2">
                      {seg ? <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${segClass(seg)}`}>{seg}</span> : <span className="text-gray-400">—</span>}
                    </td>
                    <td className="py-3 px-2 font-mono text-xs text-violet-500 dark:text-violet-400">{r.category || '—'}</td>
                    <td className="py-3 px-2">
                      {r.sentiment_label ? <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${segClass(r.sentiment_label)}`}>{r.sentiment_label}</span> : <span className="text-gray-400">—</span>}
                    </td>
                    <td className="py-3 px-2"><PayloadFormatter payload={r.payload} /></td>
                    <td className="py-3 px-2 text-xs text-gray-400">{fmtDate(r.processed_at)}</td>
                    <td className="py-3 px-2">
                      <button
                        onClick={() => handleDelete(r.submission_id || r.id)}
                        className="p-1 text-gray-400 hover:text-red-500 dark:hover:text-red-400 rounded transition"
                      >
...
export function RawDataView() {
  ...
  const fetchData = async (force = false) => {
    setLoading(true);
    try {
      const data = await fetchWithCache(`/raw-submissions?limit=${limit}`, force);
      setRows(data.rows || []);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm(`Delete raw submission #${id}?`)) return;
    try {
      const res = await fetch(`/submissions/${id}`, { method: 'DELETE' });
      if (res.ok) {
        const updated = rows.filter(r => r.id !== id);
        setRows(updated);
        const cacheKey = `/raw-submissions?limit=${limit}`;
        if (apiCache[cacheKey]) {
          apiCache[cacheKey].rows = updated;
        }
        setToastMsg(`Successfully deleted #${id}`);
        setTimeout(() => setToastMsg(null), 3000);
      }
    ...
    <div className="bg-white dark:bg-gray-800 shadow-sm rounded-xl border border-gray-200 dark:border-gray-700 p-5">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6">
        <div>
          <h2 className="text-lg font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2">
            Raw Submissions
            <span className="text-xs bg-amber-100 dark:bg-amber-500/20 text-amber-600 dark:text-amber-400 px-2 py-0.5 rounded-full">{filteredRows.length} rows</span>
          </h2>
        ...
        <div className="flex items-center gap-3 mt-3 sm:mt-0">
          <span className="text-xs text-gray-400 font-medium">Limit:</span>
          <select
            value={limit}
          ...
          <button onClick={fetchData} className="p-2 bg-gray-50 hover:bg-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600 rounded-lg text-gray-600 dark:text-gray-300 transition">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 1-1 20H4M20 20v-5h-.581m0 0a8.003 8.003 0 01-15.357-2" /></svg>
          </button>
        </div>
      </div>
      ...
      {loading ? <LoadingState /> : filteredRows.length === 0 ? <EmptyState message="No raw submissions found." /> : (
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">
            ...
            <tbody className="divide-y divide-gray-150 dark:divide-gray-700/55 text-sm text-gray-600 dark:text-gray-300">
              {filteredRows.map((r) => (
                <tr key={r.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-700/10">
                  <td className="py-3 px-2 font-mono text-xs text-gray-400">#{r.id}</td>
                  <td className="py-3 px-2">
                    <span className="px-2 py-0.5 bg-gray-100 dark:bg-gray-700 rounded text-xs font-medium text-gray-700 dark:text-gray-300">{r.form_type}</span>
                  </td>
                  <td className="py-3 px-2"><PayloadFormatter payload={r.payload} /></td>
                  <td className="py-3 px-2 text-xs text-gray-400">{fmtDate(r.created_at)}</td>
                  <td className="py-3 px-2">
                    <button
...
export function NpsView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const json = await fetchWithCache(`/analytics/nps?days=${days}`);
        setData(json);
      } catch (e) {
        console.error(e);
      } finally {
  ...
  if (loading) return <div className="bg-white dark:bg-gray-800 p-5 rounded-xl border border-gray-200 dark:border-gray-700"><LoadingState /></div>;
  ...
  const P = dist.find(x => x.name === 'Promoters') || { value: 0, pct: 0 };
  const Pa = dist.find(x => x.name === 'Passives') || { value: 0, pct: 0 };
  const D = dist.find(x => x.name === 'Detractors') || { value: 0, pct: 0 };
...
export function ScoresView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const json = await fetchWithCache(`/analytics/scores?days=${days}`);
        setData(json);
      } catch (e) {
        console.error(e);
      } finally {
  ...
  const maxS = Math.max(...rows.map(r => Number(r.avg_primary) || 0), 1);
            ...
            <table className="w-full text-left border-collapse text-sm">
              <thead>
                <tr className="border-b border-gray-200 dark:border-gray-700 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">
                  ...
                  <th className="py-2.5 px-2">Min</th>
                  <th className="py-2.5 px-2">Max</th>
                  <th className="py-2.5 px-2">P25</th>
                  <th className="py-2.5 px-2">P50 (Median)</th>
                  <th className="py-2.5 px-2">P75</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-150 dark:divide-gray-700/55 text-gray-600 dark:text-gray-300 font-mono text-xs">
                {rows.map((r) => (
                  <tr key={r.form_type} className="hover:bg-gray-50/50 dark:hover:bg-gray-700/10">
                    <td className="py-3 px-2 font-sans font-bold text-gray-800 dark:text-gray-100">{r.form_type}</td>
                    <td className="py-3 px-2">{r.count}</td>
                    <td className="py-3 px-2">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-bold font-sans ${scoreClass(r.avg_primary)}`}>{r.avg_primary}</span>
                    </td>
                    <td className="py-3 px-2">{r.min_primary || '—'}</td>
                    <td className="py-3 px-2">{r.max_primary || '—'}</td>
                    <td className="py-3 px-2">{r.p25 != null ? Number(r.p25).toFixed(1) : '—'}</td>
                    <td className="py-3 px-2">{r.p50 != null ? Number(r.p50).toFixed(1) : '—'}</td>
                    <td className="py-3 px-2">{r.p75 != null ? Number(r.p75).toFixed(1) : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
export function CategoriesView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const json = await fetchWithCache(`/analytics/categories?days=${days}`);
        setData(json);
      } catch (e) {
        console.error(e);
      } finally {
...
export function PriorityView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const json = await fetchWithCache(`/analytics/priority?days=${days}`);
        setData(json);
      } catch (e) {
        console.error(e);
      } finally {
      ...
      {keys.length === 0 ? <EmptyState message="No priority data found" /> : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {keys.map((ft) => {
            ...
            const maxC = Math.max(...frows.map(r => Number(r.count)), 1);
            return (
              <div key={ft} className="bg-gray-50 dark:bg-gray-900/30 rounded-xl p-5 border border-gray-200/50 dark:border-gray-700/50">
                <h3 className="text-sm font-bold text-gray-800 dark:text-gray-200 mb-4 uppercase tracking-wider font-mono border-b border-gray-200 dark:border-gray-700 pb-2">{ft}</h3>
                <div className="space-y-4">
...
export function VolumeView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const json = await fetchWithCache(`/analytics/volume?days=${days}`);
        setData(json);
      } catch (e) {
        console.error(e);
      } finally {
  ...
  const maxVal = Math.max(...selectedRows.map(r => Number(r.total || 0)), 1);

  const types = data.formTypes || [];
  const totals = {};
  selectedRows.forEach(day => {
    types.forEach(ft => {
      totals[ft] = (totals[ft] || 0) + (day[ft] || 0);
    });
  });
  const sortedTypes = [...types].sort((a, b) => totals[b] - totals[a]);
...
export function SentimentView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const json = await fetchWithCache(`/analytics/sentiment?days=${days}`);
        setData(json);
      } catch (e) {
        console.error(e);
      } finally {
...
export function ChurnView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const json = await fetchWithCache(`/analytics/churn?days=${days}`);
        setData(json);
      } catch (e) {
        console.error(e);
      } finally {
  ...
  const maxC = Math.max(...rows.map(r => Number(r.count)), 1);
...
export function WinLossView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const json = await fetchWithCache(`/analytics/winloss?days=${days}`);
        setData(json);
      } catch (e) {
        console.error(e);
      } finally {
  ...
  const maxO = Math.max(...(data.outcomes || []).map(r => Number(r.count)), 1);
  const maxR = Math.max(...(data.lossReasons || []).map(r => Number(r.count)), 1);
  const maxP = Math.max(...(data.pricePerception || []).map(r => Number(r.count)), 1);
...
export function PricingView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const json = await fetchWithCache(`/analytics/pricing?days=${days}`);
        setData(json);
      } catch (e) {
        console.error(e);
      } finally {
  ...
  const maxPerc = Math.max(...(data.perception || []).map(r => Number(r.count)), 1);
  const maxB = Math.max(...(data.blockers || []).map(r => Number(r.count)), 1);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 shadow-sm rounded-xl border border-gray-200 dark:border-gray-700 p-5">
          ...
          {!(data.blockers?.length) ? <EmptyState message="No blockers data available" /> : (
            <div className="space-y-4">
              {data.blockers.filter(r => r.blocker).map(r => (
                <div key={r.blocker}>
                  <div className="flex justify-between text-xs text-gray-600 dark:text-gray-400 mb-1 font-medium">
...
export function InsightsView() {
  ...
  const fetchInsights = async (force = false) => {
    setLoading(true);
    try {
      const data = await fetchWithCache('/insights/latest', force);
  ...
  const triggerInsights = async () => {
    ...
    setToastMsg(null);
    try {
      // Invalidate insights cache before triggering new ones
      delete apiCache['/insights/latest'];
      const res = await fetch('/insights/trigger', { method: 'POST' });
      const data = await res.json();
      if (data.success) {
        setToastMsg({ text: '✅ AI Insights generated successfully!', type: 'success' });
        fetchInsights(true);
      } else {
        setToastMsg({ text: `⚠️ Generation failed: ${data.error || 'Unknown error'}`, type: 'error' });
      }
    } catch (e) {
      setToastMsg({ text: '❌ Network or Server Error triggering insights.', type: 'error' });
    } finally {
      setGenerating(false);
  ...
  return (
    <div className="bg-white dark:bg-gray-800 shadow-sm rounded-xl border border-gray-200 dark:border-gray-700 p-5">
      ...
      {loading ? <LoadingState /> : insights.length === 0 ? (
        ...
        <div className="space-y-4">
          {insights.map((i, idx) => {
            ...
            const submissionIds = rawContent.submission_ids ?? i.submission_ids ?? [];
            const formType = rawContent.form_type ?? i.form_type ?? null;
                    ...
                    <div className="lg:col-span-7 space-y-3">
                      ...
                      {summary?.key_points && Array.isArray(summary.key_points) ? (
                        <ul className="space-y-2.5">
                          {summary.key_points.map((pt, pIdx) => (
                  ...
                  {per_form_type && Object.keys(per_form_type).length > 0 && (
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                        {Object.entries(per_form_type).map(([form, item], fIdx) => {
                          const trendClasses = {
